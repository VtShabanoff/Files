...
data class ContactWithGroupsEntity(
    @Embedded val contactEntity: EntityContact,
    @Relation(
        parentColumn = ContactContract.Columns.ID,
        entityColumn = GroupContract.Columns.ID,
        associateBy = Junction(ContactGroupCrossRefEntity::class)
    )
    val groupsEntity: List<GroupEntity>
)
